...
public class CropOverlayView extends View {
    ...
    private void initCropWindow() {
        ...
        float temp;
        float width = BitmapUtils.getRectWidth(mBoundsPoints);
        float height = BitmapUtils.getRectHeight(mBoundsPoints);
        temp = Math.min(BitmapUtils.getRectWidth(mBoundsPoints), BitmapUtils.getRectHeight(mBoundsPoints));
        if(width > height)
        {
            topLimit = 0.f;
            leftLimit = (width - temp ) / 2;
            rightLimit = leftLimit + temp;
            bottomLimit = BitmapUtils.getRectBottom(mBoundsPoints);
        } else {
            leftLimit = 0.f;
            topLimit = (height - temp ) / 2;
            rightLimit = BitmapUtils.getRectRight(mBoundsPoints);
            bottomLimit = topLimit + temp;
        }
