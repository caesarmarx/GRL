package grl.com.httpRequestTask;

/**
 * Created by Administrator on 6/6/2016.
 */

import android.app.Activity;
import android.app.ProgressDialog;
import android.util.Log;

import com.koushikdutta.async.future.FutureCallback;
import com.koushikdutta.ion.Ion;

import org.json.JSONException;
import org.json.JSONObject;

import java.io.File;

import grl.com.configuratoin.Constant;
import grl.com.configuratoin.GlobalVars;
import grl.com.configuratoin.SelfInfoModel;
import grl.com.network.HttpCallback;

public class FileUploadBackgroundTask extends Object implements HttpCallback {

    HttpCallback callback = null;
    Activity parent = null;

    ProgressDialog waitingDialog;
    public FileUploadBackgroundTask(Activity context, final String filePath, String fileType, final HttpCallback callback) {
        this.callback = callback;
        try {
            File file = new File(filePath);
            String contentType = "image/png";
            if (fileType.compareTo(Constant.MSG_IMAGE_TYPE) == 0) {
                contentType = "image/png";
            }
            if (fileType.compareTo(Constant.MSG_VIDEO_TYPE) == 0) {
                contentType = "video/mp4";
            }
            if (fileType.compareTo(Constant.MSG_VOICE_TYPE) == 0) {
                contentType = "audio/x-aac";
            }

            Ion.with(context)
                    .load(GlobalVars.FILE_UPLOAD)
                    .setMultipartParameter("session_id", SelfInfoModel.sessionID)
                    .setMultipartParameter("user_id", SelfInfoModel.userID)
                    .setMultipartFile("upfile", contentType, file)
                    .asString()
                    .setCallback(new FutureCallback<String>() {
                        @Override
                        public void onCompleted(Exception e, String s) {

                            if(waitingDialog != null)
                            {
                                waitingDialog.dismiss();
                            }
                            try {
                                JSONObject jsonObj = GlobalVars.getJsonDataFromString(s);
                                JSONObject jsonResultData = jsonObj.getJSONObject("result_data");
                                Log.e("json result", jsonResultData.toString());
                                Boolean result = jsonObj.getBoolean("result_code");
                                callback.onResponse(result, jsonResultData);
                                int a;
                            } catch (JSONException e1) {
                                e1.printStackTrace();
                            }
                        }
                    });
        } catch (Exception e) {

        }
    }

    @Override
    public void onResponse(Boolean flag, Object response) {
      Log.e("login response: ", response.toString());
    }
}
