 ...
 */
public class LessonMaterialSetTask extends Object implements HttpCallback {
    HttpCallback callback = null;
    Activity parent = null;

    public LessonMaterialSetTask(final Activity context, final String userId, String coursePath, final HttpCallback callback) {
        this.callback = callback;
        this.parent = context;
        try {
            Ion.with(context)
                    .load(GlobalVars.LESSON_MATERIAL_SET_URL)
                    .setTimeout(Constant.HTTP_TIME_OUT)
                    .setBodyParameter("session_id", SelfInfoModel.sessionID)
                    .setBodyParameter("user_id", userId)
                    .setBodyParameter("course_path", coursePath)
                    .asString()
