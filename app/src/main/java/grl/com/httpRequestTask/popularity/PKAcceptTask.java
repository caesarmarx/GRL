...
public class PKAcceptTask extends Object implements HttpCallback {

    HttpCallback callback = null;
    Activity parent = null;

    ProgressDialog waitingDialog;
    public PKAcceptTask(Activity context, String userId, String toUserId, String estName, int acceptValue, final HttpCallback callback) {
        this.callback = callback;
        this.parent = context;
        try {
            GlobalVars.showWaitDialog(parent);
            Ion.with(context)
                    .load(GlobalVars.PK_ACCEPT_URL)
                    .setTimeout(Constant.HTTP_TIME_OUT)
                    .setBodyParameter("session_id", SelfInfoModel.sessionID)
                    .setBodyParameter("user_id", userId)
                    .setBodyParameter("from_userid", toUserId)
                    .setBodyParameter("est_name", estName)
                    .setBodyParameter("accept_value", String.valueOf(acceptValue))
                    .asString()
