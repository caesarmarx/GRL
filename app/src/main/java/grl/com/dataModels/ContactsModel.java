...
import grl.com.App;
import grl.com.activities.energy.PhoneContactsActivity;
 ...
 */
public class ContactsModel extends Object {
    ...
    public ContactsModel(Activity context) {
                ...
                while (cur.moveToNext()) {
                    ...
                    if (Integer.parseInt(cur.getString(cur.getColumnIndex(ContactsContract.Contacts.HAS_PHONE_NUMBER))) > 0) {
                        ...
                        while (pCur.moveToNext()) {
                            ...
                            if(phone.equals(""))
                                continue;
                            contacts.contactPhone.add(phone);
                        }
                        pCur.close();
                        if(contacts.contactPhone.size() == 0)               // 전화번호가 존재하지 않는 경우
                            continue;
                        String image_uri = cur.getString(cur.getColumnIndex(ContactsContract.CommonDataKinds.Phone.PHOTO_URI));
                        if(image_uri != null)
                            contacts.contactPhoto = image_uri;
                    ...
                    if(contacts.contactPhone.size() > 0)
                        contactsList.add(contacts);
                }

                // send BroadCast
                if((context instanceof PhoneContactsActivity) || (context instanceof ContactsSelectActivity)) {                      // 신능량에서 주소록 처리 진행
                    Intent intent = new Intent(Constant.NEW_ENERGY_CONTACT);
